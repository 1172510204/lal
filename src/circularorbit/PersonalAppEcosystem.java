...
public class PersonalAppEcosystem<CentralObject, PhysicalObject>
   ...
   * @throws DependIncorrectException 没有使用App定义就在UsageLog、 InstallLog、UninstallLog、Relation
   *         中使用App的标签 防御策略：assert判断字符串不能为空
   */
  @Override
  public void readFile(String filename) throws SameLabelException, DependIncorrectException {
    ...
    Pattern pattern2 = Pattern.compile("(App ::= <)([A-Za-z0-9]+),"
        + "([A-Za-z0-9]+),([A-Za-z0-9.-_]+),\"([A-Za-z0-9 ]+)\",\"([A-Za-z0-9 ]+)\"");
    Pattern pattern3 = Pattern.compile("(InstallLog ::= <)"
        + "([0-9]{4}-[0-9]{2}-[0-9]{2}),([0-9]{2}:[0-9]{2}:[0-9]{2}),([A-Za-z0-9]+)");
    Pattern pattern4 = Pattern.compile("(UsageLog ::= <)"
        + "([0-9]{4}-[0-9]{2}-[0-9]{2}),([0-9]{2}:[0-9]{2}:[0-9]{2}),([A-Za-z0-9]+),([1-9][0-9]*)");
    Pattern pattern5 = Pattern.compile("(UninstallLog ::= <)"
        + "([0-9]{4}-[0-9]{2}-[0-9]{2}),([0-9]{2}:[0-9]{2}:[0-9]{2}),([A-Za-z0-9]+)");
  ...
  public void putPhysicalObjectOnCircles(String exact) {
    ...
    if (period.equals("Month")) {
      ...
      // exact:2019-01-01 获得此日期所在的周的模型（周一到周日为一周）
      Pattern pattern = Pattern.compile("[0-9]{4}-[0-9]{2}-[0-9]{2}");
      Matcher matcher = pattern.matcher(exact);
      ...
      for (Date d : upStall.get(app)) {
        if (d.before(end)) {
          for (Entry<String, Date> entry : exit.entrySet()) {
            if (entry.getKey().equals(app) && entry.getValue().before(d)) {
              // 最新的安装时间在卸载之前
              exit.remove(app);
              break;
            }
    ...
    for (String app : usage.keySet()) {
      if (exit.containsKey(app)) {
        for (Entry<Date, Integer> entry : usage.get(app).entrySet()) {
          if (entry.getKey().before(end) && entry.getKey().after(start)) {
            // 在规定的月内
