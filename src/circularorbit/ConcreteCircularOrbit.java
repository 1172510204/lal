...
public class ConcreteCircularOrbit<CentralObject, PhysicalObject>
  ...
  // AF
  // ConcreteCircularOrbit包含中心物体centerObject、轨道和对应的轨道物体circles、
  // 中心物体和轨道物体的关系centreTrackObject、
   ...
   * .添加中心物体
   * 
   * @param centralObject 给定要添加的中心物体
   */
  public void addCenterObject(CentralObject centralObject) {
    centerObject = centralObject;
  }
   ...
   */
  public boolean addTrackObject(int n, int r, PhysicalObject object) {
    // 必须先添加轨道才能添加轨道上的物体
    assert r >= 0 : "要删除轨道的半径小于0";
    for (Track each : circles.keySet()) {
      if (each.getR() == r) {
   ...
   * @return true if指定轨道上原本存在要删除的物体,false if要删除的物体或者指定得到轨道不存在
   */
  // 防御策略:assert判断轨道半径大于等于0
   ...
   */
  // 防御策略：assert 判断返回值的list里的track是从内到外排好
